...
module.exports = function (filePath, moduleStd, injections) {
    ...
    CallExpression(path) {
      ...
      if (callee.name === 'describe') {
        ...
        if (index !== -1) {
          ...
          // 存在新增 spec
          if (branch.length > 0) {
            const specAstArray = createSpecAsts(moduleStd[val].type, branch, suiteBlocks[val].specs);
  ...
  if (keys.length > 0) {
    keys.forEach(key => {
      ...
      const specAstArray = createSpecAsts(moduleStd[key].type, moduleStd[key].branch, suiteBlocks[key].specs, true);
      specAstArray.forEach(spec => {
